	...
	({ onSelectTemplate, onProjectLoad }, ref) => {
		...
		useEffect(() => {
			if (!editorRef.current || !isGrapesJSLoaded) {
				console.log(
					"Editor initialization skipped - missing requirements:",
					{
			...
			if (!selectedTemplate && !editorContent.bodyContent) {
				console.log(
					...
					{
						selectedTemplate,
						hasBodyContent: !!editorContent.bodyContent,
					}
				);
				return;
			}
			...
			console.log("Initializing GrapesJS editor with:", {
				selectedTemplate,
				bodyContent: editorContent.bodyContent,
			...
			try {
				...
				grapesEditorRef.current.on("component:selected", () => {
					console.log("Element selected in editor");
				});
			} catch (error) {
